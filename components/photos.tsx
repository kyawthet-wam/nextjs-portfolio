      ...
      <dialog id={photos.toString()} className="modal">
          ...
          <form method="dialog">
            ...
            <div className="w-full relative px-10">
              <span className="text-slate-500 absolute right-2 text-xs md:text-sm md:right-4">{current}/{photos.length}</span>
              <div className="scroll-container snap-mandatory snap-x w-full flex overflow-x-scroll">
                {photos.map((photo,index) => {
                  return (  
                    <div key={index} className="w-full snap-center flex-none img-container">
                      <img 
                        src={photo}
                        alt="Image"
